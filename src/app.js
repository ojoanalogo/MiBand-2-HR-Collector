...
class MiBand {
  ...
  async connect() {
    if (!bluetooth) {
      this.log("This browser doesn't support bluetooth web API 🚫");
      return false;
    }
    try {
  ...
  async startBandInit() {
    try {
      ...
      this.band.on("heart_rate", (rate) => {
        ...
        this.log(
          '<span class="has-text-danger">❤</span> Heart Rate: ' +
            rate +
            " || Time: " +
  ...
  stopMedition() {
    const csv = this.convertArrayOfObnpectsToCSV(this.dataStore);
